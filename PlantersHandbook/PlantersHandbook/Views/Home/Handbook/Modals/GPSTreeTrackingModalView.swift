...
///GPSTreeTrackingModalView.swift - View for GPSTreeTrackingModalViewController
class GPSTreeTrackingModalView: ProgrammaticViewController {
    internal var googleMapsLayout = SUI.view(backgoundColor: .systemBackground)
    internal var actionLayout = SUI.view(backgoundColor: .systemBackground)
    
    internal let engageTrackingButton = PHUI.button(title: "Start Planting", fontSize: FontSize.large)
    internal let distanceTravelledTextField = SUI.label(title: "0 m", fontSize: FontSize.extraLarge)
    internal let treesPlantedTheoreticallyTextField = SUI.label(title: "0 trees", fontSize: FontSize.extraLarge)
    internal let treesPerPlotTextField = SUI.textFieldUnderlined(placeholder: "Trees Per Plot", textType: .none)
    internal let undoImage = UIImageView(image: UIImage(named: "undoBtn.png"))
    internal let timerView = SUI.textViewRoundedBackground(text: "00:00:00", fontSize: FontSize.large)
    ...
    ///Configuire all views in google maps layout
    private func setUpGoogleMapsLayout(){
        ...
        let topBar = SUI.view(backgoundColor: .clear)
        ...
        GeneralFunctions.adjustUITextViewHeight(arg: timerView)
        timerView.translatesAutoresizingMaskIntoConstraints = false
        timerView.anchor(top: topBar.topAnchor, leading: topBar.leadingAnchor, bottom: nil, trailing: nil, padding: .init(top: 8, left: 5, bottom: 0, right: 0))
        undoImage.isUserInteractionEnabled = true
    }
    
    ///Configuire all views in action layout
