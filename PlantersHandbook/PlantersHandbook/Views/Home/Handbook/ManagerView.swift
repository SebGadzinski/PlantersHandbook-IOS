...
///ManagerView.swift - View for ManagerViewController
class ManagerView: TableViewActionView {
    
    internal let titleLabel = SUI.label(title: "", fontSize: FontSize.largeTitle)
    internal let nameTextField = SUI.textFieldUnderlined(placeholder: "", textType: .name)
    internal let addButton = PHUI.button(title: "Add Something", fontSize: FontSize.large)
    internal var managerTableView = SUI.tableViewEditable()
